...
// security.txt
app.get(`${config.baseUrl}.well-known/security.txt`, (req, res) => {
...
app.post(`${config.baseUrl}sendMail`, cors(corsOptions), function (req, res, next) {
  let data = req.body;
  let retentionDate = new Date(Date.now() + (parseInt(data.retention) * 1000));
  let htmlcontent = "<b>Neuer Upload verfügbar:</b>"
    + "<br>Betreff: " + data.topic?.toString()
    + "<br>Name: " + data.name?.toString()
    + "<br> E-Mail: " + data.email.toString()
    + "<br> Nachricht: " + data.message?.toString()
    + "<br> Downloadlink: https://upload.adonia.de" + config.baseUrl.toString() + data.sid.toString()
    + "<br> Wird gelöscht am: " + retentionDate.toLocaleDateString("de-DE").toString();
  data.files.forEach((file) => { htmlcontent = htmlcontent + "<br> Dateiname: " + file.name.toString() + " Kommentar: " + file.comment?.toString() + " Größe: " + file.humanSize.toString() }); // html body
  async function sendMail() {
    try {
      ...
      let receivers = (process.env.DEV == 'TRUE') ? process.env.MAIL_TO_DEV
      : (data.topic == req.translations.topicApplication) ? process.env.MAIL_TO_TOPIC_APPLICATION
      : (data.topic == req.translations.topicDriverLicense) ? process.env.MAIL_TO_TOPIC_DRIVERLICENSE
      : (data.topic == req.translations.topicConcertHall) ? process.env.MAIL_TO_TOPIC_CONCERTHALL
      : (data.topic == req.translations.topicSouvenirMovie) ? process.env.MAIL_TO_TOPIC_SOUVENIRMOVIE
      : (data.topic == req.translations.topicSocialMedia) ? process.env.MAIL_TO_TOPIC_SOCIALMEDIA
      : (data.topic == req.translations.topicMiscellaneous) ? process.env.MAIL_TO_TOPIC_MISCELLANEOUS
      : process.env.MAIL_TO;
      let info = await transporter.sendMail({
        from: process.env.MAIL_FROM, // sender address
        to: receivers, // list of receivers
        replyTo: data.email,
        subject: "Neuer Upload", // Subject line
        // TODO: beautify the data display in mail
        text: "Neuer Upload verfügbar. " + JSON.stringify(data), // plain text body
        html: htmlcontent,
      });
